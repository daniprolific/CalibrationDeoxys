...
class Avantes:
    ...
    def count_distribution(self, integration_time , min_wavelength = 380 , max_wavelength = 950):
    ...
    def power_distribution(self, DarkData, integration_time, min_wavelength = 380 , max_wavelength = 950):
        """Convert raw counts (Count / nm) to µW/(mm²·nm) using this formula
        ...
        ScopeData = self.count_distribution(integration_time, min_wavelength, max_wavelength)

        ADCFactor = 0.0025 # change to get correct units
    
        calib = np.ctypeslib.as_array(self.calibration_factors)
        calib_convers = calib[0:2048]  # COMPROBAR VALORES
    ...
    def power_value(self, power_dist, wavelength, min_wavelenght, max_wavelenght):
        """Calculate total power in µW/mm² performing an integration in the power distribution"""

        #CREATE A MASK IN THE DEFINED WAVELENGTH RANGE
        mask_wavelenght = (wavelength > min_wavelenght) & (wavelength < max_wavelenght)

        #APPLY THE MASK ON THE WAVELENGTHS AND THE MEASUREMENTS
        wavelengths_int = wavelength[mask_wavelenght]
        power_dist_masked = power_dist[mask_wavelenght]

        power = np.trapz(power_dist_masked, wavelengths_int)
    ...
    def measure_power(self, num_measurements , integration_time , min_wavelength , max_wavelength):
        ...
        for measure in range (num_measurements):
            power_distribution = self.power_distribution(integration_time)
            power_val = self.power_value(power_distribution, min_wavelength, max_wavelength)
    ...
    def plot_power_dist(self, power_dist):
        ...
        plt.ylabel("Power Distribution [µW/(mm²·nm)]")
...
if __name__ == "__main__":
    ...
    spectrometer.initialize_device()
    DarkData_total = np.load(r"C:\Users\madel\Desktop\Calibration\Code\Deoxys calibration\spectrometer\DarkData_30.npy")
    count = spectrometer.count_distribution(30)
    DarkData = DarkData_total[spectrometer.mask_wavelenght]
    power_dist, wave= spectrometer.power_distribution(DarkData, 30)
    print(len(power_dist))
    print(len(wave))
    # power = spectrometer.power_value(power_dist, 300,800)
